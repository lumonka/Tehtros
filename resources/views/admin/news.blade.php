...
@section('title') {{ 'Техтрос - Все новости' }} @endsection
@section('content')
    <section class="h-screen flex-1 flex bg-gray-100 dark:bg-gray-700 transition duration-500 ease-in-out overflow-y-auto px-5">
        <div class="mx-10 my-2 w-full">
            ...
            <h2 class="my-4 text-4xl font-semibold dark:text-gray-400">Список новостей</h2>
            <div class="pb-2 flex items-center justify-start text-gray-600 dark:text-gray-400 border-b dark:border-gray-600">
                <div>
                    <span class="mr-4">Всего новостей: <span class="text-orange-500 font-medium">{{ count($newsTable) }}</span></span>
                </div>
            </div>
            <div class="mt-6 flex justify-between text-gray-600 dark:text-gray-400">
                <div class="ml-10 flex capitalize">
                    <span class=" flex items-center">Изображение
                        <a href="?filter=name">
                    ...
                    <span class="ml-36  flex items-center">Описание
                ...
                <div class="flex">
                    <span class="ml-24 mr-24 flex items-center">Дата
                        <a href="?filter=price">
                            <svg class="ml-1 h-5 w-5 fill-current" viewBox="0 0 24 24">
							<path d="M18 21l-4-4h3V7h-3l4-4 4 4h-3v10h3M2 19v-2h10v2M2 13v-2h7v2M2 7V5h4v2H2z"></path>
						</svg>
            ...
            @foreach($newsTable as $news)
                <div class="mt-2 mb-4 flex px-4 py-4 justify-between bg-white dark:bg-gray-600 shadow rounded cursor-pointer hover:shadow-xl hover:rounded-lg hover:scale-105 transition">
                    <div class="flex justify-between">
                        <img class="h-12 w-12 rounded-full object-cover" src="{{ Vite::asset('resources/inc/img/') . $news->image }}" alt="Product Image"/>
                        <div class="ml-4 flex flex-col capitalize text-gray-600 dark:text-gray-400 w-80">
                            <span>Наименование</span>
                            <span class="mt-2 text-black dark:text-gray-200 line-clamp-1 text-break w-[65%]">{{ $news->name }}</span>
                        </div>
                        <div class="ml-12 flex flex-col text-gray-600 dark:text-gray-400">
                            <span>Описание</span>
                            <span class="mt-2 text-black dark:text-gray-200 line-clamp-1 w-[500px]">{{ $news->description }}</span>
                        </div>
                    </div>
                    <div class="flex">
                        <div class="mr-16 flex flex-col text-gray-600 dark:text-gray-400">
                            <span>Дата</span>
                            <span class="mt-2 font-medium text-red-600">{{ $news->created_at }}</span>
                        </div>
                    </div>
                </div>
            @endforeach
